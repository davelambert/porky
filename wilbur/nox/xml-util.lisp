...
(defmacro with-loop&read-char ((char stream &optional (eof-error-p)) &body body)
  `(loop (let ((,char (read-char ,stream ,eof-error-p nil t))) ,@body)))
...
;;;   STRING DICTIONARY
;;;
;;;   Some care must be taken when using this, since (in the interest making the
